...
Broken local audio links produce a library warning and block affected playback until repaired, even if Spotify is linked. Press `u` to rescan and review missing or renamed audio links, then choose replacement audio. Relinking an unclaimed file preserves the existing track ID and history. Intentionally absent local links are allowed. See [Spotify setup](src/PlaylistMaker.Charm/SPOTIFY_SETUP.md) for authentication, pause catch-up, persistent tracking holds, recovery controls, and timeout behaviour.

The catalogue picker loads candidates once per opening and filters in memory after a 150 ms typing pause. `Ctrl+U` clears its search. Results are grouped into catalogue tracks and unlinked local audio. `Ctrl+O` shows unused catalogue records, which remain stored for history and recovery. Unlinked audio is limited to configured audio folders; explicit catalogue links remain available outside those folders.
